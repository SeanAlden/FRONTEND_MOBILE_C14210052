...
// class untuk mengambil data dari respon API transaksi
class TransactionResponse {
  ...
  // mengambil data api transaksi beserta dengan statusnya 
  factory TransactionResponse.fromJson(Map<String, dynamic> json) {
    final details = json['details'] as List<dynamic>? ?? [];
    final statusHistories = json['status_histories'] as List<dynamic>? ?? [];

    return TransactionResponse(
      // menggunakan model Transaction untuk memanggil data-data transaksi
      transaction: Transaction(
        ...
        transactionDate: json['transaction_date'] != null
            ...
            : DateTime(2000, 1, 1),
        isFinal: json['is_final'] ?? '',
        details: details.map((e) => TransactionDetail.fromJson(e)).toList(), // menggunakan model TransactionDetail untuk memanggil data detail transaksi terkait
        statusHistories: statusHistories
            .map((e) => TransactionStatusHistory.fromJson(e))
            .toList(), // menggunakan model TransactionStatusHistory untuk memanggil data riwayat status dari transaksi terkait
      ),
    );
  }
}
